...
use super::{Error, Identifier, Parse, Parser};
...
impl TypeExpression {
  ...
  fn parse_parenthesized(parser: &mut Parser) -> Result<Self> {
    ...
    loop {
      subtypes.push(parser.consume::<TypeExpression>()?);
  ...
  fn parse_pointer(parser: &mut Parser) -> Result<Self> {
    ...
    let base = parser.consume::<TypeExpression>()?;
...
impl Tokenize for TypeExpression {
